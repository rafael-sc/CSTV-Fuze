...
class MatchesRepositoryTest {
    ...
    @Test
    fun `getMatches returns mapped matches when API call is successful`() = runTest {
        // Given
    ...
    @Test
    fun `getMatches returns empty list when API returns empty response`() = runTest {
        // Given
    ...
    //
    @Test
    fun `getMatches throws exception when API call fails`() = runTest {
        // Given
    ...
    @Test
    fun `getMatches handles HTTP error response correctly`() = runTest {
        // Given
        coEvery { matchesApi.getMatches(any(), any()) } returns Response.error(
            404,
            mockk(relaxed = true)
        )
    ...
    @Test
    fun `repository handles null values in DTO mapping correctly`() = runTest {
        // Given
        val matchDtoWithNulls = MatchDto(
            id = 1,
            name = "Test Match",
            status = "running",
            beginAt = "2025-07-20T15:00:00Z",
        ...
        coEvery { matchesApi.getMatches(any(), any()) } returns Response.success(
            listOf(
                matchDtoWithNulls
            )
        )
    ...
    @Test
    fun `repository handles pagination parameters correctly`() = runTest {
        // Given
        coEvery { matchesApi.getMatches(any(), any()) } returns Response.success(emptyList())
    ...
    @Test
    fun `repository handles different match statuses correctly`() = runTest {
        // Given
        val matchDtos = listOf(
